...
/*
   ...
   DESCRIÇÃO...........:TRADICIONAL JOGO DA VELHA ONDE O OBJETIVO DO JOGO É FAZER UMA SEQUÊNCIA DE "X" OU DE "O". O JOGADOR DEVE INFORMAR O NUMERO DA LINHA E DA COLUNA ONDE DESEJA FAZER A JOGADA.
*/
main()
{	
	...
	do
   {
	...
	for(i = 0; i <= 3; i++)
	{
		for(j = 0; j <= 3; j++)
	   {
			...
			verifica[i][j]=' '; //esvazia os vetore.	
	   }
	}
	do
	{
		...
		for(i = 1; i <= 3; i++)
	   {
			...
			for(cont = 1; cont <= 15; cont++)
		   {
				...
				if(cont==5||cont==10)
				printf("%c", -59);//coloca o caractere '+' na junção das linhas.	
		   }
			printf("\n");
	   }
		if(venceu == 1)
		...
		if(empate == 1)
	   {
		...
		if(jogada %2 != 0)//Impar = jogador 1, Par = jogador2.
	   {
		...
		do
	   {
			...
			if(linha < 1) printf("Onumero da linha não pode ser menor que 1.\n");
